...
def test_inline_refs_short(simple, config):
...
def test_inline_refs_plurals(simple, config):
    config['inline_refs'] = "short"
    config['plurals'] = "en"
    html = mock.render_single(simple, config)

    dl = xpath.dl()
    dl = dl.has(xpath.dt.has(xpath.a(id=get_id("plurals", "grandchild", "defs", 0), text="grandchild")))
    dl = dl.has(xpath.dd(text="*grandchild").has(xpath.a(title="Hello",
                                                         href="../simple.md#" + get_id("plurals",
                                                                                       "grandchildren", "refs", 0),
                                                         text="*[1]")))
    assert len(html.xpath(str(dl))) == 1


def test_inline_refs_list(simple, config):
    config['inline_refs'] = "list"
    html = mock.render_single(simple, config)
